...
public class World implements Cleanupable {
	...
	public World(WorldScene3D world) {
		...
		CellMaterial playerMaterial = cache.loadOrGetMaterial("playerMaterial", CellShader.CellMaterial.class,
				cache.loadOrGetSingleTexture(CellShader.CellMaterial.PLAYER_TEXTURE_NAME,
						CellShader.CellMaterial.PLAYER_TEXTURE_PATH));
		Mesh playerMesh = Mesh.newQuad("playerMesh", playerMaterial, new Vector2f(1));
		this.player = new CellEntity("player", cache, playerMesh,
				new CellDescriptor("player", CellType.PLAYER, "noname", null, null, null, 1));
	...
	private static List<CellDescriptor> loadCellDescriptorPool() {
		...
		try {
			JSONObject obj = new JSONObject(
					new String(Files.readAllBytes(Paths.get("./resources/gd/cells/cells.json"))));

			for (String k : obj.keySet()) {
				JSONObject sobj = obj.getJSONObject(k);
				pool.add(new CellDescriptor(k, sobj.getEnum(CellType.class, "type"), sobj.getString("scientific"),
						PDRUtils.loadRangeFloat(sobj, "hostility"), PDRUtils.loadRangeFloat(sobj, "fertility"),
	...
	private List<Entity> genCells(Vector2f center, float halfSquareSize, int numPoint) {
		...
		for (Entry<CellDescriptor, ArrayList<Vector2f>> e : cellDesc.entrySet()) {
			...
			CellInstanceEmitter emit = new CellInstanceEmitter(desc.getId() + "-part-" + center, poss.size(),
					desc.loadOrGetMaterial(cache), new Transform3D());
			cache.addMesh(emit.getParticleMesh());
			cache.addInstanceEmitter(emit);

			/*
			 * try { Files.write(Paths.get("./resources/bakes/noise/sizes.txt"), poss.parallelStream().map(t ->
			 * Math.clamp(0.6f, 1.2f, (float) ((1 - hostilityGen.noise(t)*0.2f) + fertilityGen.noise(t) * 0.3f +
			 * humidityGen.noise(t)))+"").collect(Collectors.joining("\n")).getBytes()); } catch (IOException e1) {
			 * e1.printStackTrace(); }
			 */

			Matrix4f[] matrices = poss.stream().map(t -> new Matrix4f().setTranslation(t.x, t.y, 0).scale(Math.clamp(
					0.6f, 1.2f,
					(float) ((1 - hostilityGen.noise(t)) + fertilityGen.noise(t) * 0.3f + humidityGen.noise(t)))))
					.collect(Collectors.toList()).toArray(new Matrix4f[poss.size()]);
			Object[] states = poss.stream().map(p -> (int) random.nextInt(desc.getTextureVariationCount()))
			...
			entities.add(new Entity(desc.getId() + "-" + center, new InstanceEmitterComponent(emit),
					new Transform3DComponent(new Vector3f(center.x, center.y, 0.4f)), new RenderComponent(4),
					new RenderComponent(8), new RenderConditionComponent(() -> new Vector3f(center.x, center.y, 0)
							.distance(((Camera3D) scene.getCamera()).getPosition()) < CULLING_DISTANCE)));
		}
	...
	private List<Entity> genToxins(Vector2f center, float halfSquareSize, int numPoint) {
		...
		WorldParticleEmitter emit = new WorldParticleEmitter("toxins-" + center, toxins.size(),
				(ToxinWorldParticleMaterial) cache
						.loadOrGetMaterial(ToxinWorldParticleMaterial.NAME, ToxinWorldParticleMaterial.class,
								cache.loadOrGetRenderShader(WorldParticleShader.NAME, WorldParticleShader.class),
								cache.loadOrGetSingleTexture(ToxinWorldParticleMaterial.TEXTURE_NAME,
										ToxinWorldParticleMaterial.TEXTURE_PATH, TextureFilter.NEAREST)),
				new Transform3D());
		cache.addMesh(emit.getParticleMesh());
		cache.addInstanceEmitter(emit);

		Matrix4f[] matrices = toxins.parallelStream().map(t -> new Matrix4f().setTranslation(t.x, t.y, 0))
				.collect(Collectors.toList()).toArray(new Matrix4f[toxins.size()]);
		Object[] sizes = toxins.stream().map(p -> Math.clamp(0.6f, 1f, (float) hostilityGen.noise(p)))
				.collect(Collectors.toList()).toArray();
		...
		return Arrays.asList(new Entity("toxins-" + center, new InstanceEmitterComponent(emit),
				new Transform3DComponent(new Vector3f(center.x, center.y, 0)), new RenderComponent(8),
				new RenderConditionComponent(() -> new Vector3f(center.x, center.y, 0)
						.distance(((Camera3D) scene.getCamera()).getPosition()) < CULLING_DISTANCE)));
	}
	...
	private List<Entity> genPlants(Vector2f center, float halfSquareSize, int numPoint) {
		...
		WorldParticleEmitter emit = new WorldParticleEmitter("plants-" + center, plants.size(),
				(PlantWorldParticleMaterial) cache
						.loadOrGetMaterial(PlantWorldParticleMaterial.NAME, PlantWorldParticleMaterial.class,
								cache.loadOrGetRenderShader(WorldParticleShader.NAME, WorldParticleShader.class),
								cache.loadOrGetSingleTexture(PlantWorldParticleMaterial.TEXTURE_NAME,
										PlantWorldParticleMaterial.TEXTURE_PATH, TextureFilter.NEAREST)),
				new Transform3D());
		cache.addMesh(emit.getParticleMesh());
		cache.addInstanceEmitter(emit);

		Matrix4f[] matrices = plants.parallelStream().map(t -> new Matrix4f().setTranslation(t.x, t.y, 0))
				.collect(Collectors.toList()).toArray(new Matrix4f[plants.size()]);
		Object[] sizes = plants.stream().map(p -> Math.clamp(0.6f, 1f, (float) humidityGen.noise(p) / 10 * 4 + 0.6f))
				.collect(Collectors.toList()).toArray();
		...
		return Arrays.asList(new Entity("plants-" + center, new InstanceEmitterComponent(emit),
				new Transform3DComponent(new Vector3f(center.x, center.y, 0.2f)), new RenderComponent(6),
				new RenderConditionComponent(() -> new Vector3f(center.x, center.y, 0)
						.distance(((Camera3D) scene.getCamera()).getPosition()) < CULLING_DISTANCE)));
	}
	...
	private CellDescriptor getRandomCellDescriptor(Vector2f pos) {
		...
		return cellDescriptorPool.parallelStream().filter((d) -> d.match(hostility, fertility, humidity)).findAny()
				.orElseGet(() -> cellDescriptorPool.get(random.nextInt(cellDescriptorPool.size())));
	}
	...
	public List<Vector2f> genCells(List<Vector2f> points) {
		List<Vector2f> rePoints = new ArrayList<Vector2f>();
		for (Vector2f point : points) {
			if (fertilityGen.noise(point) > 0.4 && hostilityGen.noise(point) > 0.2 && humidityGen.noise(point) > 0.3
