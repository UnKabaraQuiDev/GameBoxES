package lu.pcy113.pdr.engine.geom.utils.xmlParser;
...
public class XmlParser {
	...
	public static XmlNode loadXmlFile(File file) {
		...
		try {
			...
			e.printStackTrace();
			System.err.println(
					"Can't find the XML file: " + file.getPath());
			System.exit(
					0);
			return null;
		}
		try {
			...
			e.printStackTrace();
			System.err.println(
					"Error with XML file format for: " + file.getPath());
			System.exit(
					0);
			return null;
		}
	...
	private static XmlNode loadNode(BufferedReader reader) throws Exception {
		...
		String[] startTagParts = getStartTag(
				line).split(
						" ");
		XmlNode node = new XmlNode(
				startTagParts[0].replace(
						"/",
						""));
		addAttributes(
				startTagParts,
	...
	private static void addAttribute(String attributeLine, XmlNode node) {
		Matcher nameMatch = ATTR_NAME.matcher(
				attributeLine);
		nameMatch.find();
		Matcher valMatch = ATTR_VAL.matcher(
				attributeLine);
		valMatch.find();
		node.addAttribute(
				nameMatch.group(
	...
	private static String getStartTag(String line) {
		Matcher match = START_TAG.matcher(
				line);
		match.find();
		return match.group(
				1);
