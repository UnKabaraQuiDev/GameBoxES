...
public class DebugOptions implements Cleanupable {
	...
	public void gizmos(CacheManager cache, Scene scene, Matrix4f projectionMatrix, Matrix4f viewMatrix, Object modelMatrix) {
		...
		if (gizmoXYZ == null) {
			gizmoXYZ = ObjLoader.loadGizmo(
					"named_xyz",
					"./resources/models/gizmos/named_XYZ.obj");
		}

		gizmoXYZ.bind();

		GL40.glPolygonMode(
				GL40.GL_FRONT_AND_BACK,
				GL40.GL_LINE);

		GizmoMaterial deb;
		if (cache.hasMaterial(
				GizmoShader.GizmoMaterial.NAME)) {
			deb = (GizmoMaterial) cache.getMaterial(
					GizmoShader.GizmoMaterial.NAME);
		} else {
			deb = (GizmoMaterial) cache.loadMaterial(
					GizmoShader.GizmoMaterial.class);
		}
		RenderShader debShader = deb.getRenderShader();

		debShader.bind();

		deb.setPropertyIfPresent(
	...
	public Vector4f wireframeColor = new Vector4f(
			1,
			0,
			0,
			1),
			bonesColor = new Vector4f(
					0,
					1,
					0,
					1);
	...
	public void wireframe(CacheManager cache, Scene scene, Mesh mesh, Matrix4f projectionMatrix, Matrix4f viewMatrix, Matrix4f transformationMatrix) {
		...
		GL40.glPolygonMode(
				GL40.GL_FRONT_AND_BACK,
				GL40.GL_LINE);
		Material deb = cache.getMaterial(
				WireframeMaterial.NAME);
		if (deb == null) {
	...
	/*
	 * public void bonesWireframe(CacheManager cache, Scene scene, MeshSkeletalAnimation msa, Matrix4f projectionMatrix,
	 ...
	 * Vector3f headPos = new Vector3f(bone.getAnimatedTransform().m03(), bone.getAnimatedTransform().m13(),
	 * bone.getAnimatedTransform().m23());
	 * 
	...
	public synchronized void end(String type) {
		try {
			Pair<Long, Long> status = statuses.remove(
					type);
			if (status == null)
				return;
			eventFileWriter.append(
					"start>" + type + ":" + status.getKey() + ":" + status.getValue() + "/end>" + System.currentTimeMillis() + ":" + System.nanoTime() + "\n");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
