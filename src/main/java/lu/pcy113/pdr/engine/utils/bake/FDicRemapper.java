...
public final class FDicRemapper {
	...
	private static void copyGlyph(BufferedImage input, Graphics2D output, FDicGlyph glyph, FDicFile dic , int res, int index, int sRes) {
		...
		//output.drawImage(input, x, y, x + res, y + res, (int) Math.rint(glyph.gettCoordX() * sRes), (int) Math.rint(glyph.gettCoordY() * sRes),
		//		(int) Math.rint((glyph.gettCoordX() + glyph.gettWidth()) * sRes), (int) Math.rint((glyph.gettCoordY() + glyph.gettHeight()) * sRes), null);
	}
