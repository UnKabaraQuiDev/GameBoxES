package lu.pcy113.pdr.engine.utils.text.fdic;
	...
	public static FDicFile load(File fdicFile) {
		...
		for (int l = 0; l < lines.length; l++) {
			...
			if(glyphs) {
				...
				glyphMap.put(Character.valueOf((char) id), new FDicGlyph((char) id, width, height, hBearingX, hBearingY,
						hAdvance, vBearingX, vBearingY, vAdvance, tCoordX, tCoordY, tWidth, tHeight));
