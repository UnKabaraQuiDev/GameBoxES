...
public class TimeGraphPlot {
	...
	public static void main(String[] args) {
		System.out.println(
				"Args: " + Arrays.toString(
						args));
		if (args.length != 1) {
			System.out.println(
					"Usage: TimeGraphPlot <file>");
			return;
		}
		...
		firstDatas: {
			...
			long startMs = Long.parseLong(
					subStartParts[1]);
			long startNano = Long.parseLong(
					subStartParts[2]);
			long endMs = Long.parseLong(
					subEndParts[0]);
		...
		for (String data : dataList) {
			...
			double startMs = Long.parseLong(
					subStartParts[1]);
			long startNano = Long.parseLong(
					subStartParts[2]);
			// System.out.println("startMs = " + startMs);
			double endMs = Long.parseLong(
					subEndParts[0]);
			long endNano = Long.parseLong(
					subEndParts[1]);
			...
			// System.out.println("New startMs = " + startMs+"; New endMs = " + endMs+"; New startNano = " + startNano+"; New
			// endNano = " + endNano);

			Color color = colorMap.computeIfAbsent(
					name,
					k -> PDRUtils.randomColor(
							false));
			...
			// System.out.println("for name: " + name+", time:"+((endMs - startMs))+"ms, quad: "+startX+" "+startY+" "+width+"
			// "+height+" "+color);
		...
		try {
			...
			System.err.println(
					"Error writing image: " + e.getMessage());
		}
	}
