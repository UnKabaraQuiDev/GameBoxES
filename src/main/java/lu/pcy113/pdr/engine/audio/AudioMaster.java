...
public class AudioMaster implements Cleanupable {
	...
	public AudioMaster() {
		...
		deviceCapabilities = ALC.createCapabilities(
				device);
		PDRLoggerUtils.log(
				deviceCapabilities);
		...
		if (!deviceCapabilities.OpenALC10) {
			throw new RuntimeException(
					"Could not open OpenAL device: OpenALC10 not supported");
		}

		GlobalLogger.info(
				"Audio Output Devices: " + getDevices());
		GlobalLogger.info(
				"Default Audio Output Device: " + getDefaultDevice());
		GlobalLogger.info(
				"Audio Output Device: " + getDevice());

		alContext = ALC10.alcCreateContext(
				device,
				(IntBuffer) null);
		checkAlcError(
				device);

		if (!(deviceCapabilities.ALC_EXT_thread_local_context && EXTThreadLocalContext.alcSetThreadContext(
				alContext))) {
			if (!ALC11.alcMakeContextCurrent(
					alContext)) {
				throw new RuntimeException(
						"Coult not set context as thread context.");
			}
		}
		checkAlcError(
				device);
		...
		GlobalLogger.info(
				"ALC_FREQUENCY: " + alcGetInteger(
						device,
						ALC11.ALC_FREQUENCY));
		GlobalLogger.info(
				"ALC_REFRESH: " + alcGetInteger(
						device,
						ALC11.ALC_REFRESH));
		GlobalLogger.info(
				"ALC_SYNC: " + alcGetBool(
						...
						ALC11.ALC_SYNC));
		GlobalLogger.info(
				"ALC_MONO_SOURCES: " + alcGetInteger(
						device,
						ALC11.ALC_MONO_SOURCES));
		GlobalLogger.info(
				"ALC_STEREO_SOURCES: " + alcGetInteger(
						device,
						ALC11.ALC_STEREO_SOURCES));
	}

	private boolean alcGetBool(long device, int param) {
		return ALC11.alcGetInteger(
				device,
				param) == ALC11.ALC_TRUE;
	}
	...
	public String getDevice() {
		return alcGetString(
				ALC11.ALC_DEVICE_SPECIFIER,
				"Could not get device.");
	}

	public List<String> getDevices() {
		return alcNullError(
				ALUtil.getStringList(
						MemoryUtil.NULL,
	...
	public boolean checkAlcError(long device) {
		return ALC11.alcGetError(
				device) != AL11.AL_NO_ERROR;
	}

	@Override
	public void cleanup() {
		ALC11.alcMakeContextCurrent(
				MemoryUtil.NULL);
		AL.setCurrentProcess(
				null);
		AL.setCurrentThread(
				null);
		MemoryUtil.memFree(
				capabilities.getAddressBuffer());
