...
public abstract class AbstractShader implements UniqueID, Cleanupable {
	...
	public AbstractShader(String name, AbstractShaderPart... parts) {
		...
		if (this.shaderProgram == -1) {
			PDRUtils.throwGLError(
					name + ": Failed to create shader program!");
		}
		...
		if (GL20.glGetProgrami(
			...
			GlobalLogger.log(
					Level.SEVERE,
					name + "(" + shaderProgram + "): " + GL20.glGetProgramInfoLog(
							this.shaderProgram,
							1024));
			this.cleanup();
			throw new IllegalStateException(
					name + "(" + shaderProgram + "): Failed to link shader program!");
		} else if (!GL40.glIsProgram(
				shaderProgram)) {
			this.cleanup();
			throw new IllegalStateException(
					name + "(" + shaderProgram + "): Is not a GL Shader Program!");
		} else {
			GlobalLogger.log(
					Level.INFO,
					"ShaderProgram " + name + " (" + shaderProgram + ") created successfully");
		}
	...
	public void setUniform(String key, Object value) {
		GameEngine.DEBUG.start(
				"r_uniforms_bind_single_lookup");
		Pair<Property<Object>, Integer> unif = uniforms.get(
				key);
		if (unif == null) {
			return;
		}
		...
		if (value instanceof Integer) {
			...
			try (MemoryStack stack = MemoryStack.stackPush()) {
				GL21.glUniformMatrix3x2fv(
						unif.getValue(),
						false,
						((Matrix3x2f) value).get(
								stack.mallocFloat(
										6)));
			}
		}
	...
	public boolean hasUniform(String name) {
		return this.uniforms.containsKey(
				name);
	}

	public boolean createUniform(String name) {
		int loc = GL20.glGetUniformLocation(
				this.shaderProgram,
				name);
		PDRUtils.checkGlError();
		...
		if (loc != -1) {
			...
			GlobalLogger.log(
					Level.SEVERE,
					"Could not get Uniform location for: " + name + " in program " + this.name + " (" + this.shaderProgram + ") (" + GL11.glGetError() + ")");
		}
	...
	public void cleanup() {
		...
		if (this.shaderProgram != -1) {
			this.parts.values().forEach(
					AbstractShaderPart::cleanup);
			GL20.glDeleteProgram(
					this.shaderProgram);
			PDRUtils.checkGlError(
					"DeleteProgram(" + shaderProgram + ") (" + name + ")");
