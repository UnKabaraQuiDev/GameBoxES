...
public class Scene2DRenderer extends Renderer<GameEngine, Scene2D> {
	...
	public void render(CacheManager cache, GameEngine engine, Scene2D scene) {
		...
		for (Entity e : scene.getEntities().values()) {
			...
			Component c = null;
			if ((c = e.getComponent(
					MeshComponent.class)) != null) {
				meshRenderer.render(
						cache,
						scene,
						(MeshComponent) c);
			} else if ((c = e.getComponent(
					GizmoComponent.class)) != null) {
				gizmoRenderer.render(
						cache,
						scene,
						(GizmoComponent) c);
			} else if ((c = e.getComponent(
					InstanceEmitterComponent.class)) != null) {
				instanceEmitterRenderer.render(
						cache,
						scene,
						(InstanceEmitterComponent) c);
			} else if ((c = e.getComponent(
					TextEmitterComponent.class)) != null) {
				textEmitterRenderer.render(
						cache,
						scene,
						(TextEmitterComponent) c);
			}
		}
