...
public class TextureRenderer implements Cleanupable, UniqueID {
	...
	public TextureRenderer(String name, CacheManager cache, Vector2i res, int colorCount) {
		...
		SingleTexture depth = new SingleTexture(
				name + "[depth]",
				res.x,
				res.y);
		depth.setDataType(
		...
		depth.setInternalFormat(
				TexelInternalFormat.DEPTH_COMPONENT);
		;
		depth.setup();
		cache.addTexture(
				depth);
		...
		for (int i = 0; i < colorCount; i++) {
			SingleTexture colorX = new SingleTexture(
					name + "[color" + i + "]",
					res.x,
					res.y);
			depth.setDataType(
	...
	public TextureRenderer(String name, List<SingleTexture> attachedTextures) {
		...
		for (SingleTexture st : attachedTextures) {
			FrameBufferAttachment type = st.getFormat().equals(
					TexelFormat.DEPTH) ? FrameBufferAttachment.DEPTH : FrameBufferAttachment.COLOR_FIRST;
			type = st.getFormat().equals(
					TexelFormat.DEPTH_STENCIL) ? FrameBufferAttachment.DEPTH_STENCIL
							: (st.getFormat().equals(
									TexelFormat.STENCIL) ? FrameBufferAttachment.STENCIL : type);
			if (type.equals(
					FrameBufferAttachment.COLOR_FIRST))
				colorOffset++;
	...
	public void cleanup() {
		framebuffer.bind();
		framebuffer.getAttachments().values().forEach(
				Texture::cleanup);
