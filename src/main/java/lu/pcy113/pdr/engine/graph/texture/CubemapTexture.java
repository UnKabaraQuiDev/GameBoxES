...
public class CubemapTexture extends Texture {
	...
	public CubemapTexture(String name, String path, int filter, int wrap) {
		super(name, path, filter, GL40.GL_TEXTURE_CUBE_MAP, wrap, 0);
		...
		for (int i = 0; i < faces.length; i++) {
			...
			channels = getColorByChannels(channels);
			if (channels == -1)
				throw new RuntimeException("Invalid channel count: " + channels);
			
			if (imageBuffer != null) {
				GL40.glTexImage2D(GL40.GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, channels, width, height, 0, channels, GL40.GL_UNSIGNED_BYTE, imageBuffer);
				super.channelType = channels;
