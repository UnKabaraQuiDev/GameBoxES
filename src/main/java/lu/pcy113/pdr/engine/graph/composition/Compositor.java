...
public class Compositor {
	...
	public void render(CacheManager cache, GameEngine engine) {
		...
		GL40.glTexImage2D(GL40.GL_TEXTURE_2D, 0, GL40.GL_R32F, width, height, 0, GL40.GL_DEPTH_COMPONENT, GL40.GL_UNSIGNED_BYTE, MemoryUtil.NULL);
		/*GL40.glTexParameteri(GL40.GL_TEXTURE_2D, GL40.GL_TEXTURE_MIN_FILTER, GL40.GL_LINEAR);
		GL40.glTexParameteri(GL40.GL_TEXTURE_2D, GL40.GL_TEXTURE_MAG_FILTER, GL40.GL_LINEAR);*/
		...
		if(GL40.glCheckFramebufferStatus(GL40.GL_FRAMEBUFFER) != GL40.GL_FRAMEBUFFER_COMPLETE) {
			Logger.log(Level.SEVERE, "Framebuffer not complete: "+GL40.glCheckFramebufferStatus(GL40.GL_FRAMEBUFFER)+", w:"+width+" h:"+height);
			return;
		}
		...
		for(String l : passes) {
			...
			if(prl == null) {
				Logger.log(Level.WARNING, "Pass Render Layer: "+prl.getId()+", not found in Cache");
				break;
			}
