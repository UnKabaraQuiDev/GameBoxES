...
public class Window implements Cleanupable {
	...
	private Vector4f background = new Vector4f(
			0,
			0.1f,
			0,
			1);
	...
	public Window(WindowOptions options) {
		...
		keyCallback = GLFWKeyCallback.create(
				(window, key, scancode, action, mods) -> callback_key(
						window,
						key,
						scancode,
						action,
		...
		joystickCallback = GLFWJoystickCallback.create(
				(jid, event) -> callback_joystick(
						jid,
						event));
		GLFW.glfwSetJoystickCallback(
				joystickCallback);

		frameBufferCallback = GLFWFramebufferSizeCallback.create(
				(window, width, height) -> callback_frameBuffer(
						window,
						width,
						height));
		...
		cursorPosCallback = GLFWCursorPosCallback.create(
				(window, sx, sy) -> callback_cursor_pos(
						window,
						sx,
						sy));
	...
	private void callback_joystick(int jid, int event) {
		if (event == GLFW.GLFW_CONNECTED) {
			GlobalLogger.log(
					Level.INFO,
					"Joystick connected: jid:" + jid + ", name:" + GLFW.glfwGetJoystickName(
							jid) + ", guid:"
							+ GLFW.glfwGetJoystickGUID(
									jid)
							+ " -> name:" + GLFW.glfwGetGamepadName(
									jid)
							+ ", joystick as gamepad:" + GLFW.glfwJoystickIsGamepad(
									jid));
		} else if (event == GLFW.GLFW_DISCONNECTED) {
			GlobalLogger.log(
					Level.INFO,
					"Joystick disconnected: jid:" + jid);
		}
	}
	...
	public boolean isJoystickPresent() {
		return GLFW.glfwJoystickPresent(
				GLFW.GLFW_JOYSTICK_1);
	}

	public float getJoystickAxis(int jid, int axis) {
		FloatBuffer fb = GLFW.glfwGetJoystickAxes(
				jid);
	...
	public boolean getJoystickButton(int jid, int btn) {
		ByteBuffer fb = GLFW.glfwGetJoystickButtons(
				jid);
	...
	public ByteBuffer getJoystickButtons(int jid) {
		return GLFW.glfwGetJoystickButtons(
				jid);
	}
	...
	public boolean getJoystickHat(int jid, int hat, byte state) {
		return getJoystickHat(
				jid,
				hat) == state;
	}

	public ByteBuffer getJoystickHats(int jid) {
		return GLFW.glfwGetJoystickHats(
				jid);
	}
	...
	public boolean shouldClose() {
		return GLFW.glfwWindowShouldClose(
				handle);
	}
	...
	public void updateOptions() {
		GLFW.glfwWindowHint(
				GLFW.GLFW_RESIZABLE,
				options.resizable ? GLFW.GLFW_TRUE : GLFW.GLFW_FALSE);
		GLFW.glfwSwapInterval(
				options.vsync ? 1 : 0);
		GLFWVidMode vidMode = GLFW.glfwGetVideoMode(
				monitor);
