...
public class FourButtonState extends Entity {
	...
	public FourButtonState(CacheManager cache, Vector3f pos) {
		if (cache.hasRenderShader(
				FourButtonStateShader.NAME)) {
			FourButtonStateShader shader = (FourButtonStateShader) cache.getRenderShader(
					FourButtonStateShader.NAME);
			this.material = (FourButtonStateMaterial) new FourButtonStateMaterial(
					shader);
			cache.addMaterial(
					material);
		} else
			this.material = (FourButtonStateMaterial) cache.loadMaterial(
					FourButtonStateMaterial.class);
