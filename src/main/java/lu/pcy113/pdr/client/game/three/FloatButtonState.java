...
public class FloatButtonState extends Entity {
	...
	public FloatButtonState(CacheManager cache, Vector3f pos) {
		if (cache.hasRenderShader(
				FloatButtonStateShader.NAME)) {
			FloatButtonStateShader shader = (FloatButtonStateShader) cache.getRenderShader(
					FloatButtonStateShader.NAME);
			this.material = (FloatButtonStateMaterial) new FloatButtonStateMaterial(
					shader);
			cache.addMaterial(
					material);
		} else
			this.material = (FloatButtonStateMaterial) cache.loadMaterial(
					FloatButtonStateMaterial.class);
