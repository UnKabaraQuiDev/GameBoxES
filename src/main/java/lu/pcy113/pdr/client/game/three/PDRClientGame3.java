...
public class PDRClientGame3 extends GameLogic {
	...
	public void init(GameEngine e) {
		...
		GameEngine.DEBUG.wireframeColor = new Vector4f(
				1f,
				0.2f,
				0.2f,
				0.2f);
		...
		Texture slotTexture = cache.loadSingleTexture(
				"single_slot",
				"./resources/textures/ui/single_slot.png");
		slotMaterial = (SlotMaterial) cache.loadMaterial(
				SlotShader.SlotMaterial.class,
				slotTexture);
		Mesh slotMesh = ObjLoader.loadMesh(
				"slot",
				slotMaterial,
		...
		FloatAttribArray faa = new FloatAttribArray(
				"hover",
				7,
				1,
				new float[3 * 5],
				false,
				1);
		slotInstMaterial = (SlotInstanceMaterial) cache.loadMaterial(
				SlotInstanceShader.SlotInstanceMaterial.class,
				slotTexture);
		Mesh slotInstMesh = ObjLoader.loadMesh(
				"slotInst",
				slotInstMaterial,
				"./resources/models/plane.obj");
		this.cache.addMesh(
				slotInstMesh);
		slotInstancer = cache.loadInstanceEmitter(
				"slotEmitter",
				slotInstMesh,
				3 * 5,
				new Transform3D(),
				faa);
		slotInstancer.update(
				(inst) -> {
					((Transform3D) inst.getTransform()).setTranslation(
							new Vector3f(
									inst.getIndex() % 3 - 1f,
									(inst.getIndex() / 3) - 2f,
									0f).mul(
											1.1f));
					System.out.println(
							"instance: " + inst.getIndex() + " > " + ((Transform3D) inst.getTransform()).getTranslation());
					((Transform3D) inst.getTransform()).updateMatrix();
				});
		slotEntityUi = new Entity(
		...
		slotEntityUi.getComponent(
				Transform3DComponent.class).getTransform().updateMatrix();

		Texture txt1 = cache.loadSingleTexture(
				"txt1",
				"./resources/textures/fonts/font1row.png",
				TextureFilter.NEAREST,
				TextureType.TXT2D);
		TextMaterial textMaterial = (TextMaterial) cache.loadMaterial(
				TextShader.TextMaterial.class,
				txt1);
		debugInfo = new TextEmitter(
				"debug_infos",
				textMaterial,
				20,
				"FPS: ",
		...
		ui.addEntity(
				...
				new Entity(
						new Transform3DComponent(
								...
								new Vector3f(
										1)),
						new TextEmitterComponent(
								debugInfo)));

		ui.addEntity(
				"left_joy",
				leftJoystick = new JoystickState(
						cache,
						new Vector3f(
								-5.5f,
								-2f,
								0)));
		leftJoystick.setColor(
				new Vector4f(
						0,
						1,
						0,
		...
		ui.addEntity(
				"right_joy",
				rightJoystick = new JoystickState(
						cache,
						new Vector3f(
								-4.5f,
								-2f,
								0)));
		rightJoystick.setColor(
				new Vector4f(
						1,
						0,
						0,
		...
		ui.addEntity(
				"left_float",
				leftZButton = new FloatButtonState(
						cache,
						new Vector3f(
								-6.2f,
								-2f,
								0)));
		leftZButton.setColor(
				new Vector4f(
						0,
						1,
						1,
		...
		ui.addEntity(
				"right_float",
				rightZButton = new FloatButtonState(
						cache,
						new Vector3f(
								-3.8f,
								-2f,
								0)));
		rightZButton.setColor(
				new Vector4f(
						1,
						0,
						1,
		...
		ui.addEntity(
				"dir_btn",
				dirButtons = new FourButtonState(
						cache,
						new Vector3f(
								-5.5f,
								-1f,
								0)));
		dirButtons.setButtons(
				new Vector4f(
						0));
		ui.addEntity(
				"xyab_btn",
				xyabButtons = new FourButtonState(
						cache,
						new Vector3f(
								-4.5f,
								-1f,
								0)));
		xyabButtons.setButtons(
				new Vector4f(
						0));

		ui.addEntity(
				"left_bumper",
				leftButton = new BooleanButtonState(
						cache,
						new Vector3f(
								-5.5f,
								-2.6f,
		...
		ui.addEntity(
				"right_bumper",
				rightButton = new BooleanButtonState(
						cache,
						new Vector3f(
								-4.5f,
								-2.6f,
		...
		this.cache.addRenderer(
				new MeshRenderer());
		this.cache.addRenderer(
				new InstanceEmitterRenderer());
		this.cache.addRenderer(
				new TextEmitterRenderer());

		this.backgroundMaterial = (FillMaterial) cache.loadMaterial(
				FillShader.FillMaterial.class);
		GenerateRenderLayer genLayer = new GenerateRenderLayer(
				"gen",
				this.backgroundMaterial);
		this.cache.addRenderLayer(
				genLayer);

		SceneRenderLayer sceneRender = new SceneRenderLayer(
				"scene",
				this.scene);
		this.cache.addRenderLayer(
				sceneRender);

		SceneRenderLayer uiRender = new SceneRenderLayer(
				"ui",
				this.ui);
		this.cache.addRenderLayer(
				uiRender);

		BoxBlurMaterial boxBlurMaterial = (BoxBlurMaterial) cache.loadMaterial(
				BoxBlurShader.BoxBlurMaterial.class);
		// cache.addRenderShader(boxBlurMaterial.getRenderShader());
		PassRenderLayer boxBlurPass = new PassRenderLayer(
				"boxBlurPass",
				boxBlurMaterial);
		cache.addRenderLayer(
				boxBlurPass);

		this.compositor = new Compositor();
		this.compositor.addRenderLayer(
				0,
				genLayer);
		this.compositor.addRenderLayer(
				1,
		...
		camera.getProjection().setPerspective(
				true);
		camera.getProjection().setFov(
				(float) Math.toRadians(
						60));

		cameraUi = (Camera3D) this.ui.getCamera();
		cameraUi.setProjection(
				new Projection(
						0.01f,
						100f,
						-0.5f,
		...
		this.ui.getCamera().getProjection().update(
				1920,
				1080);
		this.scene.getCamera().getProjection().setPerspective(
				true);
		this.engine.getWindow().onResize(
				(w, h) -> {
					System.out.println(
							"resize update: " + w + "x" + h);
					this.scene.getCamera().getProjection().update(
							w,
							h);
					// this.ui.getCamera().getProjection().update(w, h);
				});
		this.engine.getWindow().setBackground(
				new Vector4f(
						0.1f));
		...
		this.textMaterialInterpolation = new Vec4fCallbackValueInterpolation<TextMaterial>(
				textMaterial,
				new Vector4f(
						1,
						0.1f,
		...
		/* CUBE MAP */
		CubemapTexture cmtxt = cache.loadCubemapTexture(
				"skybox",
		...
		scene.addEntity(
				...
				.getTransform().scaleMul(
						new Vector3f(
								10))
				.updateMatrix();

		RenderShader skeleShader = new RenderShader(
				"skeleShader",
				AbstractShaderPart.load(
						"./resources/shaders/animation/skeletal.vert"),
				AbstractShaderPart.load(
						"./resources/shaders/animation/skeletal_diffuse.frag")) {
			@Override
			public void createUniforms() {
				...
				createUniform(
						ArmatureAnimation.BONE_UNIFORM);
				createUniform(
						"diffuseColor");
			}
		};
		cache.addRenderShader(
				skeleShader);
		Material skeleMat = new Material(
				"skeleMat",
				skeleShader);
		cache.addRenderShader(
				skeleShader);
		...
		Mesh model = ObjLoader.loadMesh(
				"model",
				slotMaterial,
				"./resources/models/anims/model.obj");
		cache.addMesh(
				model);
		scene.addEntity(
				"model",
		...
		Pair<Mesh, ArmatureAnimation> pmaa = ColladaeLoader.loadMeshArmature(
				"skeleMesh",
				skeleMat,
				"./resources/models/anims/model.dae");
		skeleMesh = pmaa.getKey();
		System.err.println(
				"start data: " + skeleMesh.getVertices().getName() + " = " + Arrays.toString(
						((Vec3fAttribArray) skeleMesh.getVertices()).getData()));
		System.err.println(
				"start data: " + skeleMesh.getIndices().getName() + " = " + Arrays.toString(
						((UIntAttribArray) skeleMesh.getIndices()).getData()));
		System.err.println(
				"start data: " + skeleMesh.getAttribs()[2].getName() + " = " + Arrays.toString(
						((Vec3fAttribArray) skeleMesh.getAttribs()[2]).getData()));
		System.err.println(
				"start data: " + skeleMesh.getAttribs()[3].getName() + " = " + Arrays.toString(
						((Vec3fAttribArray) skeleMesh.getAttribs()[3]).getData()));
		ArmatureAnimation armatureAnimation = pmaa.getValue();
		System.err.println(
				armatureAnimation.getRootBone().toString(
						0));
		cache.addMesh(
				skeleMesh);
		Animation animation = ColladaeLoader.loadAnimation(
				"./resources/models/anims/model.dae");
		armatureAnimation.getAnimator().doAnimation(
				animation);
		armatureAnimation.getAnimator().update(
				0.1f);

		skeleEntity = scene.addEntity(
				"skelet",
				new Entity(
						new MeshComponent(
								skeleMesh),
						new Transform3DComponent(),
						new ArmatureAnimationComponent(
								armatureAnimation)))
				.setActive(
						true)
	...
	public void input(float dTime) {
		camera.getPosition().add(
				(window.isKeyPressed(
						GLFW.GLFW_KEY_Q) ? 0.1f : 0)
						- (window.isKeyPressed(
								GLFW.GLFW_KEY_D) ? 0.1f : 0),
				(window.isKeyPressed(
						GLFW.GLFW_KEY_R) ? 0.1f : 0)
						- (window.isKeyPressed(
								GLFW.GLFW_KEY_F) ? 0.1f : 0),
				(window.isKeyPressed(
						GLFW.GLFW_KEY_Z) ? 0.1f : 0)
						- (window.isKeyPressed(
								GLFW.GLFW_KEY_S) ? 0.1f : 0));
		camera.updateMatrix();

		if (window.isJoystickPresent()) {
			if (!window.updateGamepad(
					0))
				return;

			GLFWGamepadState gps = window.getGamepad();

			float ax = PDRUtils.applyMinThreshold(
					gps.axes(
							GLFW.GLFW_GAMEPAD_AXIS_LEFT_X),
					threshold);
			float ay = PDRUtils.applyMinThreshold(
					gps.axes(
							GLFW.GLFW_GAMEPAD_AXIS_LEFT_Y),
					threshold);
			float abtn = gps.buttons(
			...
			leftJoystick.setPosition(
					new Vector2f(
							ax,
							ay));
			leftJoystick.setButton(
					abtn);
			leftJoystick.setThreshold(
					threshold);

			float bx = PDRUtils.applyMinThreshold(
					gps.axes(
							GLFW.GLFW_GAMEPAD_AXIS_RIGHT_X),
					threshold);
			float by = PDRUtils.applyMinThreshold(
					gps.axes(
							GLFW.GLFW_GAMEPAD_AXIS_RIGHT_Y),
					threshold);
			float bbtn = gps.buttons(
			...
			rightJoystick.setPosition(
					new Vector2f(
							bx,
							by));
			rightJoystick.setButton(
					bbtn);
			rightJoystick.setThreshold(
					threshold);

			float lzb = PDRUtils.applyMinThreshold(
					gps.axes(
							GLFW.GLFW_GAMEPAD_AXIS_LEFT_TRIGGER),
					threshold);
			float rzb = PDRUtils.applyMinThreshold(
					gps.axes(
							GLFW.GLFW_GAMEPAD_AXIS_RIGHT_TRIGGER),
					threshold);

			leftZButton.setValue(
					(float) (lzb / 2 + 0.5));
			rightZButton.setValue(
					(float) (rzb / 2 + 0.5));

			float lb = gps.buttons(
					GLFW.GLFW_GAMEPAD_BUTTON_LEFT_BUMPER);
			float rb = gps.buttons(
					GLFW.GLFW_GAMEPAD_BUTTON_RIGHT_BUMPER);

			leftButton.setValue(
					lb);
			rightButton.setValue(
					rb);

			float btn_y = gps.buttons(
					GLFW.GLFW_GAMEPAD_BUTTON_Y);
			float btn_b = gps.buttons(
					GLFW.GLFW_GAMEPAD_BUTTON_B);
			float btn_a = gps.buttons(
					GLFW.GLFW_GAMEPAD_BUTTON_A);
			float btn_x = gps.buttons(
			...
			float dir_up = gps.buttons(
					GLFW.GLFW_GAMEPAD_BUTTON_DPAD_UP);
			float dir_down = gps.buttons(
					GLFW.GLFW_GAMEPAD_BUTTON_DPAD_DOWN);
			float dir_left = gps.buttons(
					GLFW.GLFW_GAMEPAD_BUTTON_DPAD_LEFT);
			float dir_right = gps.buttons(
	...
	public void update(float dTime) {
		...
		this.backgroundMaterialInterpolation.set(
				this.GX);
		this.textMaterialInterpolation.set(
				this.GX);

		slotEntity.getComponent(
				Transform3DComponent.class).getTransform().getRotation().rotateXYZ(
						0.01f,
						0.01f,
						0.01f);
		slotEntity.getComponent(
		...
		hover = org.joml.Math.clamp(
				0,
				1,
				hover);

		NextTask nt = createTask(
				GameEngine.QUEUE_RENDER);
		nt.exec(
				(status) -> {
					slotInstancer.update(
							(inst) -> {
								...
								debugInfo.setText(
										...
										.updateText();
								textEntity.getComponent(
										TextEmitterComponent.class).getTextEmitter(
												cache)
										.setText(
												"updated... " + debugInfo.getText())
										.updateText();
							});
					return status;
				}).push();
		...
		skeleEntity.getComponent(
				Transform3DComponent.class).getTransform().setScale(
						new Vector3f(
								GX * 0.1f))
				.updateMatrix();
