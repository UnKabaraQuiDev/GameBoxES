...
public class PDRClientGame4 extends GameLogic {
	...
	public void init(GameEngine e) {
		...
		cache.addGizmo(
				ray);
		rayEntity = scene.addEntity(
				"ray",
				new GizmoComponent(
						ray),
				new Transform3DComponent());

		PlainMaterial cubeMat = (PlainMaterial) cache.loadMaterial(
				PlainShader.PlainMaterial.class);
		cubeMat.setColor(
				new Vector4f(
						1));
		Mesh cube = Mesh.newCube(
				"cube",
				cubeMat,
				new Vector3f(
						1)); // cache.loadMesh("cube", cubeMat,
								// "./resources/models/cube2.obj");
		cache.addMesh(
				cube);
		defaultCube = scene.addEntity(
				"defaultCube",
				new MeshComponent(
						cube),
				new Transform3DComponent());

		Gizmo axis = ObjLoader.loadGizmo(
				"grid_xyz",
				"./resources/models/gizmos/grid_xyz.obj");
		cache.addGizmo(
				axis);
		scene.addEntity(
				"grid_xyz",
		...
		Gizmo cone = ObjLoader.loadGizmo(
				"cone",
				"./resources/models/gizmos/Ycone.obj");
		cache.addGizmo(
				cone);
		scene.addEntity(
				"cone",
		...
		this.ui.getCamera().getProjection().update(
				1920,
				1080);
		((Camera3D) this.scene.getCamera()).lookAt(
				new Vector3f(
						-5,
						0,
						0),
		...
		engine.getWindow().setBackground(
				new Vector4f(
						0.1f));
		...
		NextTask nt = new NextTask(
				...
				-1,
				super.getTaskEnvironnment(),
				worker);
		nt.exec(
				(s) -> {
					System.err.println(
							"SYSOUT Thread exec: " + Thread.currentThread().getName());
					return 0;
				}).then(
						(s) -> {
							System.err.println(
									"SYSOUT Thread callback: " + Thread.currentThread().getName());
							worker.block();
							System.err.println(
									"SYSOUT Thread callback: blocked " + Thread.currentThread().getName() + " " + worker.isBlocking());
							try {
								worker.shutdown();
								System.err.println(
										"SYSOUT Thread callback: shutdown " + Thread.currentThread().getName() + " " + worker.isActive());
			...
			createTask(GameEngine.QUEUE_RENDER)
					.exec(
							(s) -> {
								...
								fb.unbind();
								SingleTexture color0 = (SingleTexture) fb.getAttachmedTexture(
										FrameBufferAttachment.COLOR_FIRST,
										0);
								MemImage img = color0.getStoredImage();
								boolean success = FileUtils.STBISaveIncremental(
										"./logs/screenshot.png",
										img);
								img.free();
								return success ? 1 : 0;
					...
					.then(
							(s) -> {
								System.err.println(
										"M Thread then: " + Thread.currentThread().getName());
								previousF = false;
								return 1;
							})
					.push();
			...
			createTask(
					...
					.then(
							(s) -> {
								...
								Ray ray = camera.projectRay(
										window.getMousePos(),
										viewport);

								Vector3f direction = ray.getDir().get(
										new Vector3f());
								direction.normalize();
								Quaternionf eulerQuaternion = new Quaternionf().rotationTo(
										new Vector3f(
												1,
												0,
												0),
								...
								System.err.println(
										camera.getPosition());
								System.err.println(
										ray.getOrigin() + " -> " + ray.getDir());

								Vector3f pos = ((Camera3D) scene.getCamera()).projectPlane(
										ray,
										GameEngine.RIGHT,
										GameEngine.UP);
	...
	public void update(float dTime) {
		backgroundMaterialInterpolation.add(
				0.01f).mod();
		defaultCube.getComponent(
				Transform3DComponent.class).getTransform().rotate(
						0.1f,
						-0.1f,
						0.05f)
				.updateMatrix();
